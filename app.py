            ...
            for area in res.get("result", {}).get("areas", []):
                for item in area.get("datas", []):
                    ...
                    if name:
                        ...
                        # 계산 공식 오류 수정: 임의 계산법을 삭제하고 네이버 실시간 원본 누적 거래대금(aa) 필드를 적용합니다.
                        aa = item.get("aa", 0) # 당일 실시간 누적 거래대금 (원 단위 데이터)
                        volume_eok = int(aa / 100000000) # 1억 원 단위로 정확히 절삭 변환
