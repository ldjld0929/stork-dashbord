...
<style>
body, .stMarkdown, .stText, .stExpander, .stSelectbox, h4 {
...
.hts-limit .stock-title {
    color: #000000 !important;
}

.hts-limit .status-color {
    color: #d32f2f !important;
    font-weight: 900;
}

.hts-limit .hts-sub-row {
...
.stExpander {
    ...
    border: 1px solid #283954 !important;
...
def format_trade_amount(amount):
    """
    거래대금 표시 함수
    원 단위 금액을 받아서 억 / 조 억 단위로 변환
    """
    try:
        amount = int(amount)
    except:
...
def fetch_market_caps(stock_map):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        future_to_stock = {
            executor.submit(fetch_single_cap, name, code): name
        ...
        try:
            ...
            for area in res.get("result", {}).get("areas", []):
                for item in area.get("datas", []):
                    ...
                    name_list = [k for k, v in stock_map.items() if v == code]
                    ...
                    # 거래대금 수정 핵심 부분
                    # aa 값이 있으면 네이버 API 실제 거래대금 사용
                    # aa 값이 없으면 거래량 x 현재가로 보정
                    api_trade_amount = item.get("aa", 0)

                    if api_trade_amount:
                        trade_amount = api_trade_amount
                    else:
                        trade_amount = aq * close
                    ...
                    if close > 0:
                        prices[name] = {
                            ...
                            "type": chg_type,
                            "diff": f"{cv:,}",
                            "volume": format_trade_amount(trade_amount),
                            "volume_raw": int(trade_amount)
                        }
...
def fetch_live_global_financial_news(stock_name):
    ...
    url = (
        ...
        f"q={encoded_name}+-site:hankyung.com+-site:sedaily.com"
    ...
    try:
        ...
        for item in root.findall(".//item"):
            ...
            if any(k in title for k in ["유료", "로그인", "회원전용", "구독"]):
                continue

            if "hankyung.com" in link or "sedaily.com" in link:
                continue
...
def get_numeric_score(sname):
    ...
    try:
        vol_val = int(info.get("volume_raw", 0))
    except:
        vol_val = 0
...
for t_name, t_val in theme_data.items():
    ...
    processed_themes[t_name] = {
        "money": format_trade_amount(total_trade_amount),
        "news": t_val["news"],
        "stocks_data": stock_list_with_score,
        "total_vol": total_trade_amount,
        "avg_rate": avg_rate
    }
...
sorted_theme_names = sorted(
    ...
    key=lambda x: (
        processed_themes[x]["avg_rate"],
        processed_themes[x]["total_vol"]
    ),
    reverse=True
)

top_rate_stocks = sorted(
    all_stocks_data,
    key=lambda x: x[1],
    reverse=True
)[:5] if all_stocks_data else []

top_vol_stocks = sorted(
    all_stocks_data,
    key=lambda x: x[2],
    reverse=True
)[:5] if all_stocks_data else []
...
if st.session_state.page_mode == "main":
    st.markdown(
        "<h3 class='notranslate' style='margin:0 0 15px 0; color:#38bdf8;'>"
        "📱 실시간 주도주 랭킹 통합 전광판 (Full 집계)"
        "</h3>",
        unsafe_allow_html=True
    )
    ...
    with col1:
        st.markdown(
            "<h4 style='color:#ffffff; font-size:16px;'>🔥 현재 장중 급등 Top 5</h4>",
            unsafe_allow_html=True
        )
        ...
        for idx, (sname, r_val, v_val, s_info) in enumerate(top_rate_stocks):
            ...
            st.markdown(
                f"""
                <div class='rank-card notranslate' style='border-left-color: {color};'>
                    <div>
                        <span class='rank-num'>{idx + 1}</span>
                        <span style='color:white; font-weight:bold;'>{sname}</span>
                    </div>
                    <div style='color:{color}; font-weight:bold;'>{sign} {s_info['rate']}</div>
                </div>
                """,
                unsafe_allow_html=True
            )
    ...
    with col2:
        st.markdown(
            "<h4 style='color:#ffffff; font-size:16px;'>💰 현재 거래대금 Top 5</h4>",
            unsafe_allow_html=True
        )
        ...
        for idx, (sname, r_val, v_val, s_info) in enumerate(top_vol_stocks):
            st.markdown(
                f"""
                <div class='rank-card notranslate' style='border-left-color: #eab308;'>
                    <div>
                        <span class='rank-num'>{idx + 1}</span>
                        <span style='color:white; font-weight:bold;'>{sname}</span>
                    </div>
                    <div style='color:#eab308; font-weight:bold;'>{s_info['volume']}</div>
                </div>
                """,
                unsafe_allow_html=True
            )
    ...
    for t_name in sorted_theme_names:
            ...
            <div class="theme-box notranslate">
                <div class="theme-top">
                    ...
                    <div style="text-align: right;">
                        <span style="color: {avg_color} !important; font-size: 13px; font-weight: bold; margin-right: 8px;">
                            평균 {avg_str}
                        </span>
        ...
        for idx, (sname, r_val, v_val, s_info) in enumerate(sorted_stocks):
            class_mode = (
                ...
                if s_info["type"] == "5" or "-" in s_info["rate"]
                else "hts-up"
            )
            ...
            cols[idx % 4].markdown(
                f"""
                <a class='notranslate' href='?stock={sname}' target='_self' style='text-decoration:none; color:inherit;'>
                    <div class='hts-card {class_mode}'>
                        <div class='hts-row' style='display:flex; justify-content:space-between;'>
                            <span class='stock-title' style='color:#ffffff; font-weight:bold; overflow:hidden; white-space:nowrap; text-overflow:ellipsis;'>
                                {sname}
                            </span>
                            <span class='status-color' style='font-weight:bold; margin-left:4px;'>
                                {sign}{s_info['rate']}
                            </span>
                        </div>

                        <div class='hts-sub-row' style='display:flex; justify-content:space-between; margin-top:4px;'>
                            <span class='status-color'>{s_info['price']}원</span>
                            <div style='text-align:right; font-size:10px;'>
                                <span style='color:#94a3b8;'>시 {s_info['mcap']}</span>
                                <span style='color:#cbd5e1;'>| 거래 {s_info['volume']}</span>
                            </div>
                        </div>
                    </div>
                </a>
    ...
    mode_color = (
        ...
        if live["type"] in ["2", "1"] and "-" not in live["rate"]
        else "#3b82f6"
    )
    ...
    st.markdown(
        ...
        <div class="detail-card notranslate">
            <div style="display:flex; justify-content:space-between; align-items:center;">
                <span style="font-size:22px; font-weight:bold; color:#f8fafc;">⭐ {tgt}</span>
                <span style="color:#64748b; font-size:14px;">(코드 {tgt_code})</span>
            </div>

            <div style="margin: 10px 0; font-size:26px; font-weight:bold; color:{mode_color};">
                {live["price"]}
                <span style="font-size:15px; color:#cbd5e1;">({live["rate"]})</span>
                <span style="float:right; font-size:13px; color:#94a3b8; margin-top:10px;">
                    시총 {live["mcap"]} | 거래대금 {live["volume"]}
                </span>
            </div>
        </div>
        """,
    ...
    st.markdown(
        "<p style='font-size:15px; font-weight:bold; color:#38bdf8; margin-top:5px;'>🔥 실시간 뉴스 피드</p>",
        unsafe_allow_html=True
    )
    ...
    for nw in fetch_live_global_financial_news(tgt):
        with st.expander(f"📌 [{nw['source']}] {nw['title']}"):
            st.markdown(
                f"<p style='color:#cbd5e1; font-size:13px; line-height:1.6;'>{nw['desc']}</p>",
                unsafe_allow_html=True
            )
