...
# 1. 페이지 설정
st.set_page_config(page_title="NXT 주도주 전광판", layout="wide")

st.markdown("""
<style>
    body, .stMarkdown { color: #ffffff !important; }
    [data-testid="stHeader"] {background: #0f141c !important;}
    body, .stApp { background-color: #0f141c !important; }
    ...
    .hts-card { background-color: #1b2636; border: 1px solid #283954; border-radius: 4px; padding: 10px 12px; margin-bottom: 8px; cursor: pointer; text-decoration: none; color: inherit; display: block; }
        ...
        for t in themes:
            ...
            for tr in soup_t.select('table.type_5 tr'):
                ...
                if td and td.a:
                    ...
                    code = td.a['href'].split('code=')[-1][:6]
                    stocks.append(s_name)
                    stock_map[s_name] = code
            data[t['name']] = {"stocks": stocks}
        return data, stock_map
    except: return {}, {}

# [핵심 개선] 병렬 처리로 시가총액 로딩 속도 향상
...
def fetch_news(stock_name):
    ...
    try:
        ...
        return [{"title": i.find('title').text, "link": i.find('link').text, "source": i.find('source').text if i.find('source') is not None else "경제속보"} for i in root.findall('.//item')[:5]]
    except: return []
...
for t_name, t_val in theme_data.items():
    ...
    for sname in t_val["stocks"]:
        ...
        stocks_list.append((sname, r_val, v_val, info))
...
if st.session_state.page_mode == "main":
    st.markdown("<h3 style='color:#38bdf8;'>📱 실시간 주도주 랭킹 전광판</h3>", unsafe_allow_html=True)
    ...
    with c1:
        ...
        for i, (s, r, v, info) in enumerate(top_rate):
            st.markdown(f"<div class='rank-card'><span>{i+1}. {s}</span><span style='color:#ef4444;'>{info['rate']}</span></div>", unsafe_allow_html=True)
    with c2:
        st.markdown("#### 💰 거래대금 Top 5")
        for i, (s, r, v, info) in enumerate(top_vol):
            st.markdown(f"<div class='rank-card'><span>{i+1}. {s}</span><span style='color:#eab308;'>{info['volume']}</span></div>", unsafe_allow_html=True)
            ...
            st.markdown(f"""
                <a href='?stock={sname}' style='text-decoration:none;'>
                    <div class='hts-card'>
                        ...
                        <div style='font-size:11px; color:#94a3b8;'>{info['price']}원 | 시총 {MCAP_DATA.get(sname, '-')} | {info['volume']}</div>
                    </div>
                </a>
            """, unsafe_allow_html=True)
else:
    ...
    st.markdown(f"<div class='detail-card'><h3>⭐ {tgt}</h3><p>현재가 {info['price']} | {info['rate']}</p><p>시총: {MCAP_DATA.get(tgt, '-')} | 거래대금: {info['volume']}</p></div>", unsafe_allow_html=True)
    st.write("🔥 실시간 뉴스 피드")
